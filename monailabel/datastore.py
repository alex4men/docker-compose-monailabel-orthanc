...
def download_label(label: str, tag: str, check_only=False):
    instance: MONAILabelApp = app_instance()
    image_id = label
    logger.info(f"Download label: {label}, with tag: {tag}")
    label = instance.datastore().get_label_info(label, tag)['SeriesInstanceUID']
    label = instance.datastore().get_label_uri(label, tag, image_id)
